...
export function StatsCard({
  ...
  return (
    <Card variant="compact" className={cn('text-center', className)}>
      {icon && (
        <div className={cn('text-2xl mb-2', colorVariants[color])}>
          {icon}
        </div>
      )}
...
export function QuickActionCard({
  ...
  const content = (
    <div className="group p-4 bg-gradient-to-br from-gray-50 to-gray-100 hover:from-white hover:to-gray-50 rounded-xl border border-gray-200 hover:border-gray-300 transition-all duration-200 hover:shadow-md cursor-pointer">
      <div className="flex items-center mb-3">
        <div className={cn(
          'w-10 h-10 bg-gradient-to-r rounded-lg flex items-center justify-center text-white text-lg mr-3 group-hover:scale-110 transition-transform duration-200',
          color
        )}>
          {icon}
        </div>
