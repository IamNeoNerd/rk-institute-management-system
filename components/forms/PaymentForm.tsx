...
export default function PaymentForm({ selectedStudentId, onSuccess, onCancel }: PaymentFormProps) {
  ...
  const fetchPendingAllocations = async (studentId: string) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`/api/allocations?studentId=${studentId}&status=PENDING`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
    ...
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      {pendingAllocations.length > 0 && (
        <div className="bg-gradient-to-r from-blue-50 to-blue-100 rounded-2xl p-6 border border-blue-200">
          <h4 className="text-xl font-bold text-gray-900 mb-4">Pending Fee Allocations</h4>
          <div className="space-y-3">
            {pendingAllocations.map((allocation) => (
              <div key={allocation.id} className="flex items-center justify-between bg-white rounded-lg p-3">
                <div className="flex items-center">
                  <input
                    type="checkbox"
                  ...
                  />
                  <label htmlFor={`allocation-${allocation.id}`} className="ml-3 text-sm font-medium text-gray-900">
                ...
                <div className="text-right">
                  <div className="font-bold text-green-600">₹{allocation.netAmount.toFixed(2)}</div>
          ...
          {selectedAllocations.length > 0 && (
            <div className="mt-4 p-3 bg-green-50 rounded-lg border border-green-200">
              <div className="flex justify-between items-center">
                ...
                <span className="text-xl font-bold text-green-900">₹{calculateTotalAmount().toFixed(2)}</span>
              </div>
            </div>
          )}
        </div>
      ...
      <div>
        <label htmlFor="amount" className="block text-sm font-semibold text-gray-700 mb-2">
          Payment Amount (₹) {selectedAllocations.length > 0 && '(Auto-calculated from selected allocations)'}
        </label>
        <input
          type="number"
          name="amount"
          ...
          step="0.01"
          className="input-field"
          value={selectedAllocations.length > 0 ? calculateTotalAmount() : formData.amount}
          onChange={handleChange}
          placeholder="0.00"
