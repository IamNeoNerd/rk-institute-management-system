  ...
  const cardContent = (
    <div className={`group relative bg-gradient-to-br ${colors.bg} p-6 rounded-2xl border ${colors.border} transition-all duration-300 hover:shadow-xl hover:-translate-y-1 ${disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'} ${className}`}>
      ...
      {action.type === 'link' && action.href ? (
        <div className="mt-4">
          <span className={`inline-flex items-center px-4 py-2 ${colors.button} text-white text-sm font-medium rounded-lg transition-all duration-200 shadow-md hover:shadow-lg`}>
            {action.label}
