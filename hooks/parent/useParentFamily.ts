...
export interface UseParentFamilyReturn {
  ...
  fetchChildProgress: (
    childId: string
  ) => Promise<ChildAcademicProgress | null>;
...
export function useParentFamily(): UseParentFamilyReturn {
  ...
  const [selectedChildData, setSelectedChildData] = useState<Child | null>(
    null
  );
  ...
  const fetchChildProgress = useCallback(
    async (childId: string): Promise<ChildAcademicProgress | null> => {
      try {
        const token = localStorage.getItem('token');
        const response = await fetch(
          `/api/parents/children/${childId}/progress`,
          {
            headers: { Authorization: `Bearer ${token}` }
          }
        );
  ...
  const updateFamilyProfile = useCallback(
    async (updates: Partial<FamilyProfile>): Promise<boolean> => {
      try {
        const token = localStorage.getItem('token');
  ...
  const addEmergencyContact = useCallback(
    async (contact: Omit<EmergencyContact, 'id'>): Promise<boolean> => {
      try {
        const token = localStorage.getItem('token');
  ...
  const getChildProgressById = useCallback(
    (childId: string): ChildAcademicProgress | null => {
      return (
        childrenProgress.find(progress => progress.childId === childId) || null
      );
    },
    [childrenProgress]
  );
