...
function validateEnvironmentVariables(vars, context = 'production') {
  ...
  REQUIRED_PRODUCTION_VARS.forEach(varName => {
    if (!vars[varName]) {
      ...
      vars[varName].includes('CHANGE_THIS') ||
      vars[varName].includes('your-domain')
    ) {
  ...
  if (vars.JWT_SECRET && vars.JWT_SECRET.length < 32) {
    issues.push(
      `❌ JWT_SECRET must be at least 32 characters (current: ${vars.JWT_SECRET.length})`
    );
  }
  ...
  if (
    ...
    !vars.NEXT_PUBLIC_APP_URL.startsWith('https://')
  ) {
    warnings.push('⚠️  NEXT_PUBLIC_APP_URL should use HTTPS in production');
  }

  // Check MCP integration variables
  const mcpVarsPresent = MCP_INTEGRATION_VARS.filter(
    varName => vars[varName]
  ).length;
  if (mcpVarsPresent > 0 && mcpVarsPresent < MCP_INTEGRATION_VARS.length) {
    warnings.push(
      `⚠️  Partial MCP integration detected (${mcpVarsPresent}/${MCP_INTEGRATION_VARS.length} variables)`
    );
  }
...
function validateCurrentEnvironment() {
  ...
  const localValidation = validateEnvironmentVariables(
    localVars,
    'local development'
  );
  ...
  // Check coverage
  const requiredCount = REQUIRED_PRODUCTION_VARS.filter(
    v => localVars[v]
  ).length;
  const optionalCount = OPTIONAL_PRODUCTION_VARS.filter(
    v => localVars[v]
  ).length;
  ...
  console.log(
    `  Required Variables: ${requiredCount}/${REQUIRED_PRODUCTION_VARS.length}`
  );
  console.log(
    `  Optional Variables: ${optionalCount}/${OPTIONAL_PRODUCTION_VARS.length}`
  );
...
function generateSyncCommands(target) {
  ...
  if (validation.issues.length > 0) {
    console.error(
      '❌ Environment validation failed. Fix issues before syncing:'
    );
  ...
  switch (target) {
    case 'vercel':
      ...
      console.log('='.repeat(50));
      console.log(
        '# Run these commands to sync environment variables to Vercel:'
      );
      console.log('');
      break;
    ...
    default:
      console.error(
        '❌ Invalid target. Use --target=vercel or --target=github'
      );
      process.exit(1);
  }
...
function generateProductionEnvTemplate() {
  ...
  console.log(
    '✅ Production environment template saved to: .env.production.template'
  );
}
...
function main() {
  ...
  console.log('Usage:');
  console.log(
    '  node scripts/sync-environment.js --validate     # Validate current environment'
  );
  console.log(
    '  node scripts/sync-environment.js --template     # Generate production template'
  );
  console.log(
    '  node scripts/sync-environment.js --target=vercel   # Generate Vercel sync commands'
  );
  console.log(
    '  node scripts/sync-environment.js --target=github   # Generate GitHub secrets commands'
  );
  console.log('');
