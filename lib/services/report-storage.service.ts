...
export class ReportStorageService {
  ...
  async storeReport(reportData: ReportData): Promise<StoredReport> {
    try {
      ...
      console.log(
        `[Report Storage] Stored ${reportData.type} report: ${report.id}`
      );
  ...
  async incrementDownloadCount(id: string): Promise<void> {
    try {
      ...
      console.log(
        `[Report Storage] Incremented download count for report: ${id}`
      );
    } catch (error) {
   ...
   */
  async createReportPlaceholder(
    reportData: Omit<ReportData, 'data'>
  ): Promise<string> {
    try {
      const report = await prisma.report.create({
        data: {
      ...
      console.error(
        '[Report Storage] Error creating report placeholder:',
        error
      );
